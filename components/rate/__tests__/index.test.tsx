...
describe('Rate', () => {
  ...
  describe('8.config provider', () => {
    it('should config use config provider', () => {
      ...
      const wrapper = mount(
        <ConfigProvider value={rateConfig}>
          <Rate />
        </ConfigProvider>,
      )
      expect(wrapper.find('.kd-rate-icon-selected').length).toBe(6)
  ...
  describe('10.api test', () => {
    it('API test', () => {
      // defaultValue
      const defaultValueRate = mount(<Rate defaultValue={1}></Rate>)
      expect(defaultValueRate.find('.kd-rate-icon-selected').length).toBe(2)
      // value
      const valueRate = mount(<Rate value={3.0}></Rate>)
      expect(valueRate.find('.kd-rate-icon-selected').length).toBe(6)
    })
  })
})
