...
const InternalClipboard = (props: IClipboardProps, ref: unknown): FunctionComponentElement<IClipboardProps> => {
  ...
  const handleClick = useCallback(() => {
    ...
    if (content) {
      ...
      if (!targetElement) {
        // 没有获取到元素，触发错误的回调
        onError?.(new Error(`target element '${target}' does not exist`))
        return
      } else {
        if (action === 'copy' && targetElement.hasAttribute('disabled')) {
          // 复制被置为 disabled 的元素，触发错误的回调
          onError?.(
            new Error(
              `invalid "target" attribute of element '${target}'. Please use "readonly" instead of "disabled" attribute`,
            ),
          )
          return
        }
        if (action === 'cut' && (targetElement.hasAttribute('readonly') || targetElement.hasAttribute('disabled'))) {
          // 剪切被置为 readonly 或 disabled 的元素，触发错误的回调
          onError?.(
            new Error(
              `invalid "target" attribute of element '${target}'. You can't cut text from elements with "readonly" or "disabled" attributes`,
            ),
          )
          return
        }
