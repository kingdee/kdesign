...
使用 `create-react-app` 一步步地创建一个 TypeScript 项目，并引入 @kdcloudjs/kdesign。

> `@kdcloudjs/kdesign` 基于最新稳定版本的 TypeScript（`>=4.0.0`），请确保项目中使用匹配的版本。
...
```bash
$ yarn create react-app kdesign-demo-ts --template typescript
```

如果你使用的是 npm（接下来我们都会用 yarn 作为例子，如果你习惯用 npm 也没问题）。

```bash
$ npx create-react-app kdesign-demo-ts --template typescript
```
...
修改 `src/App.tsx`，引入 @kdcloudjs/kdesign 的按钮组件。
...
修改 `src/App.css`，在文件顶部引入 @kdcloudjs/kdesign 的样式。

```css
@import '@kdcloudjs/kdesign/dist/kdesign.css';
```

重新启动 `yarn start`，现在你应该能看到页面上已经有了 @kdcloudjs/kdesign 的按钮组件，接下来就可以继续选用其他组件开发应用了。其他开发流程你可以参考 create-react-app 的[官方文档](https://create-react-app.dev/docs/getting-started#creating-a-typescript-app)。

`@kdcloudjs/kdesign` 使用 TypeScript 书写并提供了完整的定义，你可以享受组件属性输入建议和定义检查的功能。
...
如果你已经按照 [在 create-react-app 中使用](/docs/guide/create-react-app) 初始化了环境，可以参考官方文档里的 [Adding TypeScript](https://create-react-app.dev/docs/adding-typescript) 配置 TypeScript 开发环境。
